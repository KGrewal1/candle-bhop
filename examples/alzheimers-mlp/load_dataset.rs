...
fn load_parquet(parquet: SerializedFileReader<std::fs::File>) -> Result<(Tensor, Tensor)> {
    ...
    // let colnames = parquet.metadata().file_metadata().schema().get_fields();
    // println!("{}", samples);
    // println!("{:?}", colnames);
    ...
    let images = (Tensor::from_vec(buffer_images, (samples, 1, 128, 128), &Device::Cpu)?
        .to_dtype(DATATYPE)?
        / 255.)?;
...
pub fn load_mnist() -> Result<(Tensor, Tensor, Tensor, Tensor)> {
    ...
    let repo = Repo::dataset("Falah/Alzheimer_MRI".to_string());
    let repo = api.repo(repo);
    // println!("{:?}", repo.info()?);

    let test_parquet_filename = repo
        .get("data/test-00000-of-00001-44110b9df98c5585.parquet")
    ...
    let train_parquet_filename = repo
        .get("data/train-00000-of-00001-c08a401c53fe5312.parquet")
        .map_err(|e| Error::Msg(format!("Api error: {e}")))?;
