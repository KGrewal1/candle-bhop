...
// impl ConvNet {
//     pub fn forward_tens(&self, input: &Tensor) -> Result<Tensor> {
//         // , train: bool
//         //xs: &Tensor,
//         let (b_sz, _i, _img_x, _img_y) = self.train_input.dims4()?;
...
impl SimpleModel for ConvNet {
    ...
    fn new(vs: VarBuilder, setup: MySetupVars) -> Result<Self> {
        ...
        let dense1 = candle_nn::linear(800, 64, vs.pp("dense1"))?;
        // let dense2 = candle_nn::linear(128, 16, vs.pp("dense2"))?;
        // let dense3 = candle_nn::linear(16, 128, vs.pp("dense3"))?;
        let dense4 = candle_nn::linear(64, 800, vs.pp("dense4"))?;
    ...
    fn test_eval(&self) -> candle_core::Result<f32> {
        // , train: bool
        //xs: &Tensor,
        let (b_sz, _i, _img_x, _img_y) = self.test_input.dims4()?;
        // let xs = self.dropout.forward_t(&xs, train)?;
        let pixels = self
            .train_input
            .reshape((b_sz, 1, 28, 28))?
